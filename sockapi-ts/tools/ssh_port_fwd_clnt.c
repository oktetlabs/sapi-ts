...
/**
 * @page tools-ssh_port_fwd_clnt Check OpenSSH TCP port forwarding work
 *
...
retrieve_pco_via_host_param(tools_lib_ssh_host host)
{
    switch(host)
    {
        ...
        default:
            TEST_FAIL("Unknown host parameter");
    }
}
...
retrieve_tst_sockaddr_via_host_param(tools_lib_ssh_host host)
{
    switch(host)
    {
        ...
        default:
            TEST_FAIL("Unknown TST host parameter");
    }
}
                                  ...
                                  tools_lib_ssh_host tst_host_param)
{
    if (host_is_iut(host_param))
        return retrieve_iut_sockaddr_via_tst_host_param(tst_host_param);
    else
        return retrieve_tst_sockaddr_via_host_param(host_param);
}
...
{
    tdata->tst = retrieve_pco_via_host_param(tester);
    tdata->tst_addr = retrieve_tst_sockaddr_via_host_param(tester);

    tdata->srv = retrieve_pco_via_host_param(server);
    CHECK_RC(tapi_sockaddr_clone(tdata->srv,
                                 retrieve_sockaddr_via_host_params(server, tester),
                                 &tdata->srv_addr));

    tdata->clnt = retrieve_pco_via_host_param(client);
...
main(int argc, char *argv[])
{
    ...
    pco_tst = retrieve_pco_via_host_param(tester);
    tst_addr = retrieve_tst_sockaddr_via_host_param(tester);
